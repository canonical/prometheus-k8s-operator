...
class MetricsEndpointProvider(Object):
    ...
    def __init__(
        ...
        auto_generate_default_job=True,
        ...
        Args:
            ...
            external_hostname: an optional argument that represents a external hostname that
                can be generated by an Ingress or a Proxy.
            auto_generate_default_job: if `jobs` is not provided, a default job would
                be auto-generated, unless this arg is set to False. This may be useful
                for use cases such as cos-config, where only rule files need to be
                forwarded with no scrape jobs specified. This is typically used when there is
                a desire to forward rules that apply globally (across all deployed charms
                 and units) rather than to a single charm.
            group_name_prefix: an optional prefix to be added to the group name of all alerts. This
                is useful for use-cases such as cos-config, where the absence of scrape metadata
                and juju topology would otherwise result in a non-unique filename.
        ...
        if jobs:
            ...
            self._jobs = [DEFAULT_JOB] if auto_generate_default_job else []
        ...
        for ev in refresh_event:
            self.framework.observe(ev, self._set_reldata_scrape_host)

        # In case the alerts included with the charms changed from one charm revision to another.
    ...
    def _set_scrape_job_spec(self, event):
        ...
        """
        self._set_reldata_alerts()
        self._set_reldata_scrape_host()
        self._set_reldata_scrape_jobs()

    def _set_reldata_alerts(self):
        if not self._charm.unit.is_leader():
            return

        app_data, _ = PrometheusScrapeRelationData.alerts(
        ...
        for relation in self._charm.model.relations[self._relation_name]:
            for k, v in app_data.items():
                relation.data[self._charm.app][k] = v

    def _set_reldata_scrape_host(self, *_):
        ...
        """
        for relation in self._charm.model.relations[self._relation_name]:
            _, unit_data = PrometheusScrapeRelationData.hostname_details(  # type: ignore
                relation,
                self._charm,
                self._scrape_jobs,
            ...
            for k, v in unit_data.items():
                relation.data[self._charm.unit][k] = v

    def _set_reldata_scrape_jobs(self):
        if not self._charm.unit.is_leader():
            return

        app_data, _ = PrometheusScrapeRelationData.scrape_details(self.topology, self._scrape_jobs)
        for relation in self._charm.model.relations[self._relation_name]:
            for k, v in app_data.items():
                relation.data[self._charm.app][k] = v
...
def _is_valid_unit_address(address: str) -> bool:
...
class PrometheusScrapeRelationData:
    """Utility class for constructing the prometheus_scrape relation data.

    The prometheus_scrape relation data consists of two components:
    - scrape information
    - accompanying alert rules

    The scrape information consists of two components:
    - scrape jobs
    ...
    @classmethod
    def alerts(
        cls, alert_rules_path, *, topology: JujuTopology = None, group_name_prefix: str = ""
    ) -> Tuple[Dict[str, str], Dict[str, str]]:
        """Returns the alerts portion of the relation data."""
        alert_rules = AlertRules(topology=topology, group_name_prefix=group_name_prefix)
        alert_rules.add_path(alert_rules_path, recursive=True)
        alert_rules_as_dict = alert_rules.as_dict()

        app_data = (
            {"alert_rules": json.dumps(alert_rules_as_dict, sort_keys=True)}
            # if alert_rules_as_dict
            # else {}
        )
        unit_data = {}  # type: Dict[str, str]
        return app_data, unit_data

    @classmethod
    def hostname_details(
        cls, relation, charm, scrape_jobs, external_hostname
    ) -> Tuple[Dict[str, str], Dict[str, str]]:
        """Returns the scrape hostname portion of the relation data."""
        app_data = {}  # type: Dict[str, str]

        # Generate hostname data only if there are scrape jobs
        if scrape_jobs:
            if external_hostname:
                unit_address = external_hostname
            else:
                unit_address = str(charm.model.get_binding(relation).network.bind_address)
                unit_address = (
                    unit_address if _is_valid_unit_address(unit_address) else socket.getfqdn()
                )
            unit_name = charm.model.unit.name
        else:
            unit_address, unit_name = "", ""

        unit_data = {
            "prometheus_scrape_unit_address": unit_address,
            "prometheus_scrape_unit_name": unit_name,
        }

        return app_data, unit_data

    @classmethod
    def scrape_details(cls, topology, scrape_jobs) -> Tuple[Dict[str, str], Dict[str, str]]:
        """Returns the scrape jobs portion of the relation data."""
        # Generate scrape details only if there are scrape jobs
        if scrape_jobs:
            app_data = {
                "scrape_metadata": json.dumps(topology.as_dict(), sort_keys=True),
                "scrape_jobs": json.dumps(scrape_jobs, sort_keys=True),
            }
        else:
            app_data = {
                "scrape_metadata": "",
