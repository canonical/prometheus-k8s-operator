...
class MetricsEndpointAggregator(Object):
    ...
    def _on_alert_rules_changed(self, event):
        ...
        unit_name = event.unit.name
        self.set_alert_rule_data(unit_name, unit_rules)

    def set_alert_rule_data(
        self, unit_name: str, unit_rules: dict, label_rules: bool = True
    ) -> None:
        ...
        """
        if label_rules:
            rules = self._label_alert_rules(unit_rules, unit_name)
        updated_group = {"name": self.group_name(unit_name), "rules": rules}
    ...
    def _on_alert_rules_departed(self, event):
        ...
        """
        group_name = self.group_name(event.unit.name)
        unit_name = event.unit.name
