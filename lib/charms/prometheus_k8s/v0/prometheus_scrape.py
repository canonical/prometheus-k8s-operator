...
The `MetricsEndpointProvider` will read all available rules and
...
Gathering rules and generating rule files within the Prometheus
charm is easily done using the `alerts()` or `recording_rules()` method(s)
of `MetricsEndpointConsumer`. Rules generated by Prometheus will
automatically include Juju topology labels in the alerts. These labels
indicate the source of the record or alert. The following labels are
automatically included with each rule
...
class Rules(ABC):
    ...
    def _multi_suffix_glob(
        self, dir_path: Path, suffixes: List[str], recursive: bool = True
    ) -> list:
    ...
    def add_path(self, path: Union[str, Path], *, recursive: bool = False) -> None:
...
class MetricsEndpointConsumer(Object):
    ...
    def jobs(self) -> list:
        ...
        scrape_jobs = self._dedupe_job_names(scrape_jobs)

        return scrape_jobs

    def _dedupe_job_names(self, jobs: List[dict]):
    ...
    def recording_rules(self) -> dict:
        """Fetch recording rules for all relations.

        A Prometheus recording rules file consists of a list of "groups". Each
        group consists of a list of recording rules (`rules`) that are sequentially
        ...
        of recording groups are indexed by that relations Juju topology identifier.
        The Juju topology identifier string includes substrings that identify
        recording rule related metadata such as the Juju model, model UUID and the
        application name from where the recording rule originates. Since this
        topology identifier is globally unique, it may be used for instance as
        the name for the file into which the list of recording rule groups are
        ...
        Returns:
            ...
            its list of recording rule groups.
        """
    ...
    def _get_rules_by_type(self, rule_type: str) -> dict:
        ...
        for relation in self._charm.model.relations[self._relation_name]:
            ...
            try:
                ...
                rules[identifier] = self._tool.apply_label_matchers(rules)
                ...
                identifier = self._get_identifier_by_rule_content(rules, rule_type)

            if not identifier:
                logger.error(
                    "%s rules were found but no usable group or identifier was present",
                    rule_type.capitalize(),
                )
                continue

            all_rules[identifier] = rules

            _, errmsg = self._tool.validate_rules(rules)
            if errmsg:
                if rules[identifier]:
    ...
    def _get_identifier_by_rule_content(self, rules: dict, rule_type: str) -> Union[str, None]:
        """Determine an appropriate dict key for rules.

        The key is used as the filename when writing rules to disk, so the structure
        and uniqueness is important.

        Args:
            rules: a dict of rules
            rule_type: a string representing the rule type for interpolation in messages
        """
        if "groups" not in rules:
            logger.debug("No %s groups were found in relation data", rule_type)
            return None

        # Construct an ID based on what's in the rules if they have labels
        for group in rules["groups"]:
            try:
                ...
                logger.debug(
                    "%s rules were found but no usable labels were present", rule_type.capitalize()
                )
                continue

        logger.warning(
            "No labeled %s rules were found, and no 'scrape_metadata' "
            "was available. Using the %s group name as filename.",
            rule_type,
            rule_type,
        )
        try:
...
def _resolve_dir_against_charm_path(charm: CharmBase, *path_elements: str) -> str:
    ...
    rules_dir_path = charm_dir.absolute().joinpath(*path_elements)

    if not rules_dir_path.exists():
        raise InvalidRulePathError(rules_dir_path, "directory does not exist")
    if not rules_dir_path.is_dir():
        raise InvalidRulePathError(rules_dir_path, "is not a directory")

    return str(rules_dir_path)
...
class MetricsEndpointProvider(Object):
    ...
    def __init__(
        ...
        This can be observed via `consumer.on.recording_rule_status_changed`, or
        `consumer.on.alert_rule_status_changed`, which contains:
            - The error(s) encountered when validating as `errors`
            - A `valid` attribute, which can be used to reset the state of charms if rules
        ...
        Args:
            ...
            recording_rules_path: an optional path for the location of recording rules
                files.  Defaults to "./prometheus_recording_rules",
                resolved relative to the directory hosting the charm entry file.
                The recording rules are automatically updated on charm upgrade.
    ...
    def _on_relation_changed(self, event):
        """Check for rule messages in the relation data before moving on."""
    ...
    def set_scrape_job_spec(self, _=None):
        ...
        recording_rules = RecordingRules(topology=self.topology)
        recording_rules.add_path(self._recording_rules_path, recursive=True)
        ...
        for relation in self._charm.model.relations[self._relation_name]:
            ...
            if recording_rules_as_dict:
                relation.data[self._charm.app]["recording_rules"] = json.dumps(
                    recording_rules_as_dict
                )
...
class PrometheusRulesProvider(Object):
    ...
    This object may be used to forward rules to Prometheus. This is unlike
    :class:`MetricsEndpointProvider`, which is used for forwarding both scrape targets and
    associated rules. This object is typically used when there is a desire to forward rules
    that apply globally (across all deployed charms and units) rather than to a single charm.
    All rule files are forwarded using the same 'prometheus_scrape' interface that is also used by
    ...
    Args:
        ...
        alert_dir_path: Root directory for the collection of rule files.
    ...
    def __init__(
        ...
        charm: CharmBase,
        relation_name: str = DEFAULT_RELATION_NAME,
        alert_dir_path: str = DEFAULT_ALERT_RULES_RELATIVE_PATH,
        recording_dir_path: str = DEFAULT_RECORDING_RULES_RELATIVE_PATH,
        recursive=True,
        **kwargs,
    ):
        super().__init__(charm, relation_name)
        ...
        # FIXME: get rid of this once cos-config stops using it, or stops using
        # explicitly named arguments. It's the only consumer.
        if "dir_path" in kwargs:
            alert_dir_path = kwargs.get("dir_path", DEFAULT_ALERT_RULES_RELATIVE_PATH)

        try:
            alert_dir_path = _resolve_dir_against_charm_path(charm, alert_dir_path)
        except InvalidRulePathError as e:
            logger.debug(
                "Invalid Prometheus alert rules folder at %s: %s",
                e.rules_absolute_path,
        ...
        self.alert_dir_path = alert_dir_path

        try:
            recording_dir_path = _resolve_dir_against_charm_path(charm, recording_dir_path)
        except InvalidRulePathError as e:
            logger.debug(
                "Invalid Prometheus recording rules folder at %s: %s",
                e.rules_absolute_path,
    ...
    def _reinitialize_alert_rules(self):
        ...
        # FIXME: cos-config uses this, and should call `reinitialize_rules` instead. We don't
        # need to keep everything private.
        self.reinitialize_rules()

    def reinitialize_rules(self):
        """Reloads rules and updates all relations."""
    ...
    def _update_relation_data(self, _):
        ...
        alert_rules.add_path(self.alert_dir_path, recursive=self._recursive)
        alert_rules_as_dict = alert_rules.as_dict()

        recording_rules = RecordingRules()
        recording_rules.add_path(self.recording_dir_path, recursive=self._recursive)
        recording_rules_as_dict = recording_rules.as_dict()
        ...
        for relation in self._charm.model.relations[self._relation_name]:
            ...
            relation.data[self._charm.app]["alert_rules"] = json.dumps(
                alert_rules_as_dict, sort_keys=True
            )

            relation.data[self._charm.app]["recording_rules"] = json.dumps(
                recording_rules_as_dict, sort_keys=True
            )
