...
from scenario import Container, Context, PeerRelation, Relation, State, Network
...
def add_relation_sequence(context: Context, state: State, relation: Relation):
    ...
    state_with_relation = state.replace(relations=state.relations + [relation], networks=state.networks + [Network.default(relation.endpoint)])
